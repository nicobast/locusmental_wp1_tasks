...
## Python Installation
* The required Python module tobii-research is compatible only with Python 3.10. Therefore, ensure you install Python 3.10 according to your system requirements or create. Afterwards, you can create a virtual environment that uses Python 3.10. 
 ...
 * "Microsoft Visual C++ 14.0" bundled with "Microsoft C++ Build Tools" (Desktop Development with C++ and/or Python Development with C++ needs to be checked).
 * FFmpeg to run the between-tasks videos. To check if FFmpeg is installed, run "ffmpeg -version". If it is not installed, download it from: https://ffmpeg.org/download.html.You can install it in Program Files or directly within your environment. If installing within the environment, you can extract the files to the following path: share\ffpyplayer\ffmpeg\bin\ffplay.exe.You need to check the runner file to ensure the path where you extracted FFmpeg matches the one specified in the list (lines: 170-173).Alternatively, the code is adapted to use VLC media player if FFmpeg is not available.
 * If you use a virtual environment, all modules must be installed within that activate environment:
  1. Psychopy: Install version 2023.1.3, as newer versions may not be compatible : "pip install psychopy==2023.1.3"
    - If you cannot install the required PsychoPy version via pip due to pypi-search package, follow this steps:
        - download psychopy manually: get it from the github releases page: https://github.com/psychopy/psychopy/releases/tag/2023.1.3
        - modify the configuration file: navigate to the PsychoPy folder, open the setup.cfg file, and delete the pypi_search from the list of required packages 
 ...
 * Issue: during task execution, an error in one of the module files occured prevented the recording of gaze data.
  ...
  - Solution: 
    1.  Locate the following file in your Python environment: <Python Environment Directory>lib\site-packages\psychopy\iohub\devices\eyetracker\hw\tobii\eyetracker.py 
      OR site-packages\psychopy\build\lib\psychopy\iohub\devices\eyetracker\hw\tobii\eyetracker.py
