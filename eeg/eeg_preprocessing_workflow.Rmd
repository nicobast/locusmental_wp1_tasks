---
title: "eeg_preprocessing_workflow"
author: "Nico Bast"
date: "`r Sys.Date()`"
output: 
  html_document:
...
library(ggplot2)
library(gridExtra)
require(eegUtils) # spatial interpolation
#require(fastICA)
library(signal)
...
takes EEG data in CSV format that has been collected with BrainProduct 32-channel R-net caps.
Raw data has been recorded with Lab Streaming Layer and combined with triggers from Psychopy.
See: convert_xdf_to_csv.py

Preprocessing includes:
- eeg data loading
- inspect channels
- bandpass filtering
- identify bad channels (noisy, flat, low intercorrelation)
- spatial interpolation with inverse distance weighting
- common average reference
- Independent Component Analysis (ICA) - with PCA and JADE algorithm
- baseline correction
...
#EEG sampling rate - not tested for others than 1000Hz
sampling_rate <- 1000
...
- recorded test subject (LENI) with Tobii TX300 and R-Net Caps.
- 32 channels with 3 movement axis (Liveamp movement)
- Raw data has already been combined with EEG triggers: see merge_eeg_with_triggers_from_csv.r.
- electrode position information is converted to cartesian coordinates and plotted for visual inspection

```{r define EEG and path}
...
#where channels are found - data also contains amplifier motion (x,y,z) 
eeg_channel_range<-2:33

#derived from manual BrainProducts RNP-LA-32.pdf
channel_labels<-c(
...
- full preprocessing into epoched data

## load and select task data

- loads data, defines channel names and provides available variables
- selects according task data
- plot shows EEG trigger distribution along the recording

```{r load data and inspect trigger}
...
names(raw_data) #variables in data set
...
ggplot(raw_data,aes(x=1:nrow(eeg_data),fill=trigger_trial))+
  geom_histogram(bins=100)+ 
  labs(title='EEG trigger distribution')+xlab('EEG recoding samples (time axis)')
# samples / sampling_rate = time in seconds

```

## bandpass filtering

- bandpass default: 1-40Hz (butterworth filter)
- also visual inspection of filtered channel by correlation plots

```{r bandpass filtering}
...
inspect_channels<-function(data){
  ...
  ggplot(plot_summary,aes(x=colnames(data),y=median))+
    geom_boxplot(aes(
      ...
      ymax = median + 3*sd),
      stat = "identity")+
    labs(y='median voltage')+xlab('electrode channel')
...
corrplot::corrplot(cor(filtered_data),type='upper',order='FPC',title='EEG channel correlations clustered')

```

## bad channel identification

- flat (SD<1), noisy (SD>100), and low correlation (r<.4) channels
...
channel_correlation<-function(data){
  ...
  low_intercorrelations<-names(mean_intercorrelations)[mean_intercorrelations<correlation_cutoff]
...
paste('Identified as bad channel:',bad_channels)
...
- requires electrode coordinates in cartesian space & bad channels
...
corrplot::corrplot(cor(filtered_data),type='upper',order='FPC',
                   title='clustered EEG channel correlation - after spatial interpolation') #check effect of spatial interpolation
...
- common average reference
- compare mean amplitude before and after re-referencing
...
print('before')
round(apply(filtered_data,2,median),2) #mean amplitude BEFORE rereferenceing
filtered_data<-apply_car(filtered_data)
print('after')
round(apply(filtered_data,2,median),2) #mean amplitude AFTER rereferenceing
...
- ICA 
- visual inspection of 5 first major components of ICA
- automated component removal with kurtosis of components
...
plot_preprocessed_segments<-function(eeg_data){
  ...
  print('plotting 10 random seconds of 10 random channels')
...
#retrieve data from output list element
...
corrplot::corrplot(cor(eeg_clean),type='upper',order='FPC',title='cleaned EEG channel correlations') #correlation ordered by first principal component (clustered correlation patterns)
...
- corrected for mean of pre-stimulus phase in epoch (-200 to 0 ms)
...
- looks into Fz electrode for P300 signal

```{r preprocessed data inspection, warnings=FALSE}
